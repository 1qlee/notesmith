...
function Handwriting({
  ...
  const { opacity, rows, spacing, strokeWidth, dashedLineData } = pageData
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let line = 0; line < 3; line++) {
        ...
        if (line === 1 && !dashedLineData.sync) {
          lineProps = {
            stroke: "#000000",
            strokeWidth: convertToPx(dashedLineData.strokeWidth),
            strokeDasharray: dashedLineData.dashArray,
            strokeDashoffset: dashOffset,
            strokeDashOpacity: `${dashedLineData.opacity}%`,
        ...
        <React.Fragment key={index}>
          {row.map((line, index) => (
            <line
              ...
              opacity={line.strokeDashOpacity ? line.strokeDashOpacity : line.opacity}
              x1={line.x1}
              x2={line.x2}
              y1={line.y1}
              y2={line.y2}
